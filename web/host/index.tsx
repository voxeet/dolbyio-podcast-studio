...
import Invitation from '../shared/services/models/invitation';
...
class Index extends Component<any, IndexState> {
    ...
    async onSessionOpened(podcastIdentifier: string) {
        ...
        console.log('Podcast Identifier:', podcastIdentifier);
        ...
        this.setState({
            isLoading: true,
            loadingMessage: 'Request access to the podcast',
        });

        try {
            const invitation: Invitation = await Backend.getInvitation(podcastIdentifier, false, externalId);

            this.setState({
                isLoading: true,
                loadingMessage: 'Joining the podcast',
                podcastName: invitation.name,
            });

            await Sdk.joinConference(invitation.identifier, invitation.accessToken);
